...
import org.gradle.api.internal.tasks.testing.DefaultParametrizedTestDescriptor;
...
public class TestEventSerializer {
    public static SerializerRegistry create() {
        ...
        registry.register(DefaultParametrizedTestDescriptor.class, new DefaultParametrizedTestDescriptorSerializer());
    ...
    @NonNullApi
    private static class DefaultParametrizedTestDescriptorSerializer implements Serializer<DefaultParametrizedTestDescriptor> {
        final Serializer<CompositeIdGenerator.CompositeId> idSerializer = new IdSerializer();

        @Override
        public DefaultParametrizedTestDescriptor read(Decoder decoder) throws Exception {
            Object id = idSerializer.read(decoder);
            String name = decoder.readString();
            String className = decoder.readNullableString();
            String displayName = decoder.readString();
            CompositeIdGenerator.CompositeId parentId = idSerializer.read(decoder);
            return new DefaultParametrizedTestDescriptor(id, name, className, displayName, parentId);
        }

        @Override
        public void write(Encoder encoder, DefaultParametrizedTestDescriptor value) throws Exception {
            idSerializer.write(encoder, (CompositeIdGenerator.CompositeId) value.getId());
            encoder.writeString(value.getName());
